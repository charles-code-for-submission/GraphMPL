...
from dgi.models.utils_promptms_2l import funcfenc,\
...
class DGI2ms2l_mi_lth(nn.Module):
    def __init__(self, n_in, n_h1, n_h2, n_way, activation, ladj, madj, hid, pemb=None, bottleout=None, absin=0, spratio=0.0):
        ...
        self.cls = nn.Linear(1 * self.hid, self.n_way, bias=True)
    ...
    def forward(self, features, seq1, adj, vars, sparse=True, bias=True, train=True):
        ...
        if self.pemb is None:
            ...
            if self.absin == 5:
                ...
                concat = mfmidl
                # 设立了两个bottle，但是只用了其中一个，sparsity也只mask一个bottle
                ...
                miloss1 = self.prompt_mi(hl, mfmidl)
                # miloss2 = self.prompt_mi(hinl, mfinmidl)
                miloss = max(0, miloss1)
